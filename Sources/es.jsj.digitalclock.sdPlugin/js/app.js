...
function connected(jsn) {
    ...
    $SD.on('es.jsj.digitalclock.action.willAppear', jsonObj =>
        action.onWillAppear(jsonObj)
    );
    $SD.on('es.jsj.digitalclock.action.willDisappear', jsonObj =>
        action.onWillDisappear(jsonObj)
    );
    $SD.on('es.jsj.digitalclock.action.keyUp', jsonObj =>
        action.onKeyUp(jsonObj)
    );
    $SD.on('es.jsj.digitalclock.action.sendToPlugin', jsonObj =>
        action.onSendToPlugin(jsonObj)
    );
}

var action = {
    type: 'es.jsj.digitalclock.action',
    cache: {},
    ...
    onSendToPlugin: function (jsn) {
        ...
        } else if (jsn.payload.hasOwnProperty('clock_index')) {
            /* if there's no clock-definitions, so simply do nothing */
...
function DigitalClock(jsonObj) {
    ...
    function drawClock(jsn) {
        ...
        clockface.text === true && $SD.api.setTitle(context, hh + ":" + mm, null);
        $SD.api.setImage(
            context,
            clock.getImageData()
        );
